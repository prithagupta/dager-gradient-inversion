...
def get_init(args, model, unused_tokens, shape, true_labels, true_grads, bert_embeddings, bert_embeddings_weight,
             tokenizer, lm, lm_tokenizer, ids, pads):
    device = lm.device
    num_inits = shape[0]
    ...
    if args.init == 'lm':
        ...
        embeds = bert_embeddings(embeds)[:, :shape[1], :].detach()
    ...
    for i in range(args.init_candidates):
        ...
        fix_special_tokens(tmp_embeds, bert_embeddings.weight, pads, is_bert=args.bert_path != 'gpt2',
        ...
        if (best_rec_loss is None) or (rec_loss < best_rec_loss):
            ...
            logger.info(f'[Init] best rec loss: {best_rec_loss.item()} for {sen}')

    # Pick best permutation of candidates
    for i in range(args.init_candidates):
        idx = torch.cat((torch.tensor([0], dtype=torch.int32), torch.randperm(shape[1] - 2) + 1,
        ...
        if (rec_loss < best_rec_loss):
            ...
            _, cos_ids = get_closest_tokens(tmp_embeds, unused_tokens, bert_embeddings_weight, metric='cos')
            sen = tokenizer.batch_decode(cos_ids)
            logger.info(f'[Init] best perm rec loss: {best_rec_loss.item()} for {sen}')
