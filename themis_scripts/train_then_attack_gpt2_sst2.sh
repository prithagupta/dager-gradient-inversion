...
DEVICE_GRAD="${DEVICE_GRAD:-cuda}"
FORCE_TRAIN="${FORCE_TRAIN:-0}"

echo "[THEMIS CONFIG] script=train_then_attack_gpt2_sst2.sh"
echo "[THEMIS CONFIG] train_noises=${noises[*]}"
echo "[THEMIS CONFIG] attack_batches=${attack_batches[*]}"
echo "[THEMIS CONFIG] attack_seeds=${attack_seeds[*]}"
echo "[THEMIS CONFIG] models=${models[*]}"
echo "[THEMIS CONFIG] datasets=${datasets[*]}"
echo "[THEMIS CONFIG] attacks=$ATTACKS"
  ...
  for dataset in "${datasets[@]}"; do
    for noise in "${noises[@]}"; do
      ...
      if [ "$FORCE_TRAIN" = "1" ] || [ -z "$finetuned_path" ] || [ ! -d "$finetuned_path" ]; then
        train_lock_path="${output_dir}.train.lock"
        mkdir -p "$(dirname "$train_lock_path")"
        exec {train_lock_fd}>"$train_lock_path"
        echo "[THEMIS TRAIN-THEN-ATTACK] Waiting for train lock: $train_lock_path"
        flock "$train_lock_fd"

        finetuned_path="$(latest_model_for_output_dir "$output_dir" || true)"
        if [ "$FORCE_TRAIN" = "1" ] || [ -z "$finetuned_path" ] || [ ! -d "$finetuned_path" ]; then
          echo "[THEMIS TRAIN-THEN-ATTACK] Training model=$model dataset=$dataset noise=$noise"
          ...
          echo "[THEMIS TRAIN-THEN-ATTACK] Reusing finetuned model after lock: $finetuned_path"
        fi

        flock -u "$train_lock_fd"
        eval "exec ${train_lock_fd}>&-"
      fi

      echo "[THEMIS TRAIN-THEN-ATTACK] Reusing finetuned model: $finetuned_path"
