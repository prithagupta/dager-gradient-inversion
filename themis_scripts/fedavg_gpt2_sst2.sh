...
echo "[THEMIS CONFIG] script=fedavg_gpt2_sst2.sh"
echo "[THEMIS CONFIG] batches=${batches[*]}"
echo "[THEMIS CONFIG] seeds=${seeds[*]}"
echo "[THEMIS CONFIG] attacks=${attacks[*]}"
echo "[THEMIS CONFIG] models=${models[*]}"
echo "[THEMIS CONFIG] datasets=${datasets[*]}"
echo "[THEMIS CONFIG] FedAvg: E=$AVG_EPOCHS lr=$AVG_LR b_mini=$B_MINI"
echo "[THEMIS CONFIG] device_grad=$DEVICE_GRAD"
...
run_attack() {
  ...
  echo ""
  echo "=================================================="
  echo "Running Themis FedAvg attack=$attack | model=$model | dataset=$dataset | batch_size=$batch | seed=$seed"
  echo "Args: ${args[*]}"
  echo "=================================================="
